...
export type SitemapSource = "s3" | "file";
...
export async function loadSitemapMarkdown(): Promise<{
  ...
  const res = await s3.client.send(
    new GetObjectCommand({
      Bucket: s3.bucket,
      Key: SITEMAP_ASSET_KEY,
    }),
  );

  const body = res.Body;
  if (!body) {
    throw new Error("S3 returned an empty sitemap object body.");
  }
